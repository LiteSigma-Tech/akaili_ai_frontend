...
<template>
    <div ref="widgetContainer"
        ...
        :style="{ width: widgetWidth + 'px', height: widgetHeight + 'px', maxWidth: '100%' }">
        <!-- Chat Header -->
        <div class="bg-[#9E4CFF] dark:bg-[#7C3AED] text-white px-6 py-4 flex justify-between items-center shrink-0 select-none cursor-move"
            @mousedown="startDrag">
            <div>
            ...
            <button v-if="messages.length > 0" @click="requestEndChat" :disabled="isEnding"
        ...
        <!-- Handover banners -->
        <div v-if="handoverStatus === 'requested'"
        ...
        <div v-if="handoverStatus === 'rejected'"
        ...
        <div v-if="handoverStatus === 'active'"
        ...
        <div v-if="sessionRestored"
            ...
            <button @click="sessionRestored = false" class="text-blue-400 hover:text-blue-600 text-xs ml-4">✕</button>
        </div>

        <!-- Messages -->
        <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-slate-950">
            <div v-if="messages.length === 0" class="text-center text-gray-500 dark:text-gray-400 mt-12">
                <div
                    class="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 inline-block max-w-xs mx-auto">
                    <p class="text-lg">👋</p>
            ...
            <div v-for="(message, index) in messages" :key="index"
                ...
                <div :class="[
                    ...
                    <div v-if="message.role !== 'user'"
                        ...
                        v-html="formatMessage(message.message)"></div>
                    ...
                    <div v-if="message.role === 'assistant' && message.sources?.length"
                        ...
                        <div class="text-xs text-gray-500 space-y-1">
                            <div v-for="(s, i) in message.sources.slice(0, 2)" :key="i">📄 {{ s.table || 'Document' }}
                            </div>
                        </div>
                    </div>
            ...
            <div v-if="isTyping" class="flex justify-start">
                <div
                    class="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 shadow-sm px-4 py-3 rounded-2xl rounded-bl-none">
                    <div class="flex space-x-1.5 items-center h-4">
                        <div v-for="i in 3" :key="i" class="w-2 h-2 bg-gray-400 rounded-full animate-bounce"
            ...
            <div v-if="showRatingModal"
                ...
                <div
                    ...
                    <div class="flex justify-center gap-2 mb-5">
                        <button v-for="star in 5" :key="star" @click="hoveredRating = star; pendingRating = star"
                    ...
                    <!-- Optional comment -->
                    <textarea v-model="ratingComment" placeholder="Any additional comments? (optional)" rows="2"
                        class="w-full px-3 py-2 text-sm border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none mb-4 placeholder-gray-400"></textarea>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button @click="submitRating" :disabled="!pendingRating || ratingSubmitting"
        ...
        <!-- Resize handles -->
        <div class="absolute right-0 top-0 bottom-4 w-3 cursor-ew-resize hover:bg-purple-500/10 z-50"
            @mousedown.prevent="startResize('right', $event)"></div>
        <div class="absolute left-0 top-0 bottom-4 w-3 cursor-ew-resize hover:bg-purple-500/10 z-50"
            @mousedown.prevent="startResize('left', $event)"></div>
        <div class="absolute left-4 bottom-0 right-4 h-3 cursor-ns-resize hover:bg-purple-500/10 z-50"
            @mousedown.prevent="startResize('bottom', $event)"></div>
        <div class="absolute bottom-0 right-0 w-6 h-6 cursor-nwse-resize z-50"
...
const persistSession = () => {
    try {
        ...
        emit('session-saved', {
            sessionId: sessionId.value,
            conversationId: conversationId.value,
            preview: messages.value.find(m => m.role === 'user')?.message?.slice(0, 60) ?? '',
...
const loadPersistedSession = () => {
    try {
        ...
        if (!p?.expiresAt || Date.now() > p.expiresAt) { localStorage.removeItem(storageKey.value); return false }
        return p
    } catch (_) { return false }
}

const clearPersistedSession = () => { try { localStorage.removeItem(storageKey.value) } catch (_) { } }
...
defineExpose({ restoreSpecificSession })
...
const formatMessage = (text) => {
    if (!text) return ''
    let html = ''; let inList = false; let listType = null
    for (const line of text.split('\n')) {
        ...
        if (/^[*\-]\s+/.test(line)) {
            if (!inList || listType !== 'ul') { if (inList) html += '</ul>'; html += '<ul class="list-disc pl-5 my-2">'; inList = true; listType = 'ul' }
            html += `<li>${p.replace(/^[*\-]\s+/, '')}</li>`
        } else if (/^\d+\.\s+/.test(line)) {
            if (!inList || listType !== 'ol') { if (inList) html += '</ol>'; html += '<ol class="list-decimal pl-5 my-2">'; inList = true; listType = 'ol' }
...
const clearConversationChannel = () => {
    if (conversationChannel && conversationId.value) {
        try { $echo.leave(`conversation.${conversationId.value}`) } catch (_) { }
...
const requestHumanAgent = async () => {
    ...
    } catch (err) { error.value = 'Failed to connect'; isHandoverRequested.value = false }
...
const sendMessage = async () => {
    ...
    } catch (e) { error.value = 'Failed to send message. Please try again.' }
    finally { isTyping.value = false; scrollToBottom() }
}
